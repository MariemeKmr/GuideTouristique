...
@section('title', 'Modifier l\'activite')
...
@section('content')
    ...
    <main class="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-nuit mb-6">Modifier l'activite</h1>

        <div class="rounded-2xl border border-sable-200 bg-white shadow-soft p-6">
            <form method="POST" action="{{ route('admin.activites.update', $activite) }}">
                @csrf
                @method('PUT')
                @include('admin.activites._form')
                <div class="mt-6 flex items-center gap-3">
                    <button type="submit" class="rounded-xl bg-terracotta px-4 py-2 text-sm font-semibold text-white shadow-soft hover:bg-terracotta-600 hover:shadow-lift transition">Mettre a jour</button>
